/* ============================================================
   PRESENTATION TIER — Employer verification dashboard.
   ============================================================ */
import React, { useState } from "react";
import { C, FONT, MONO } from "../theme.js";
import { Badge, Btn, Card, Field, FingerprintStrip, SectionTitle } from "./ui.jsx";
import { canonicalize, computeDataHash, computeRecordHash, extractAcademicData, readFileWithHash } from "../../application/crypto.js";
...
function EmployerView({ user, entries, records, tokens, checks, setChecks, log }) {
  ...
  const verifyToken = async () => {
    ...
    if (Date.now() > tk.expiresAt) { setResult({ ok: false, msg: "Token has expired. Ask the credential holder to generate a fresh QR code." }); record("failed", "qr_scan", null); setBusy(false); return; }
  ...
  const verifyDoc = async () => {
    ...
    const typedId = idIn.trim().toUpperCase();
    ...
    if (typedId && extracted && extracted.studentId && extracted.studentId !== typedId)
      return fail(`Identity mismatch — the document names ${extracted.studentId}, but you entered ${typedId}. Flagged for potential forgery.`);

    const studentRecords = records.filter((r) => String(r.structured.studentId).toUpperCase() === wantedId && entries.find((e) => e.id === r.entryId && e.status === "certified"));
  ...
  return (
    <div>
      ...
      <Card>
        <div className="flex gap-2 mb-4">
          <Btn small kind={mode === "qr" ? "primary" : "ghost"} onClick={() => { setMode("qr"); setResult(null); setShowScanner(true)}}>Scan QR (paste token)</Btn>
    ...
    />
            ...
            <Field label="Verification ID / token — or the hash from a downloaded document's QR seal">
              <input className={inputCls} style={{ ...inputStyle, fontFamily: MONO }} value={tokenIn} onChange={(e) => setTokenIn(e.target.value)} placeholder="qr_… or EDUVERIFY-PNG|SHA256:…" />
            </Field>
          ...
          <>
            <Field label="Candidate's SevisPass ID number (optional — cross-checked against the document)">
              <input className={inputCls} style={{ ...inputStyle, fontFamily: MONO }} value={idIn} onChange={(e) => setIdIn(e.target.value)} placeholder="SP-1003" />
            </Field>
            ...
            <Field label="Or paste the certificate text">
              <textarea className={inputCls} style={{ ...inputStyle, minHeight: 120, fontFamily: MONO, fontSize: 12 }} value={docIn} onChange={(e) => setDocIn(e.target.value)} placeholder="Paste the certificate or transcript text here (Ctrl+V / Cmd+V)…" />
            </Field>
            <p className="text-xs mb-3" style={{ color: C.gray }}>
              How it works: the system extracts the key academic information from the document — student ID, institution, programme, graduation year, and GPA — builds a standardized representation of that data, hashes it with SHA-256, and compares it against the hash generated from the official academic record on file. Matching hashes verify the qualification as authentic; a mismatch flags the document for potential forgery or alteration.
            </p>
              ...
              {result.ok && result.rec && (
                <div className="mt-3 p-3 rounded text-sm" style={{ background: C.paper }}>
                  {[
                    ...
                    ["Student", `${studentName(result.rec.structured.studentId)} (${result.rec.structured.studentId})`],
