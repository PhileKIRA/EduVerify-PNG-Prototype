...
function Login({ group, users, onBack, onPick, onRegister }) {
  ...
  const walletScan = (p) => { setWho(p); setStage("done"); };

  return (
    <div className="pt-8">
      <div className="flex justify-center mb-5">
        <img src={LOGO_FULL} alt="EduVerify PNG" style={{ width: 190, maxWidth: "58%", height: "auto" }} />
      </div>
      ...
      <Card>
        ...
        {stage === "start" && (
          <div>
            ...
            <p className="text-xs mt-3" style={{ color: C.gray }}>
              Protocol: OIDC4VP (OpenID Connect for Verifiable Presentations). {SEVISPASS_CONFIG.mock ? "Prototype mode — the SevisPass server is simulated." : `Connected to ${SEVISPASS_CONFIG.serverUrl}.`}
            </p>
            {group === "institution" && (
        ...
        {stage === "register" && (
          <div>
            ...
            <Field label="Registrar / admin user (signs in with SevisPass)">
              <input className={inputCls} style={inputStyle} value={reg.registrar} onChange={(e) => setReg({ ...reg, registrar: e.target.value })} placeholder="e.g. PIT Registry Office" />
            </Field>
        ...
        {stage === "qr" && session && (
          <div className="flex gap-5 flex-wrap items-start">
            ...
            <div className="shrink-0" style={{ border: `1px solid ${C.line}`, borderRadius: 8, padding: 8, background: "#fff" }} dangerouslySetInnerHTML={{ __html: session.qrCode }} />
