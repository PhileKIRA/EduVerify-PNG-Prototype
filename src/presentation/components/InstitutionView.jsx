...
function InstitutionView({ user, institutions, entries, setEntries, records, setRecords, tokens, setTokens, log }) {
  ...
  // Defensive: a registrar account must resolve to a real institution. If it
  // doesn't (e.g. the institution was removed, or the UID is not yet bound),
  // show a clear message instead of crashing on inst.status below.
  if (!inst)
    return (
      <Card>
        <Badge status="pending" />
        <p className="text-sm mt-3" style={{ color: C.inkSoft }}>
          This registrar account isn't linked to an institution yet. Ask the system administrator to confirm your institution has been approved and that your SevisPass UID is assigned as its registrar.
        </p>
      </Card>
    );
  ...
  const runSearch = () => {
    const id = q.trim();
    const student = PERSONAS.find((p) => p.role === "student" && sameSevisId(p.id, id));
  ...
  const entryRow = (e, action) => (
    <Card key={e.id}>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="font-bold text-sm" style={{ color: C.ink }}>{studentName(e.studentId)} <span style={{ fontFamily: MONO, color: C.gray, fontWeight: 400 }}>({formatSevisId(e.studentId)})</span></div>
      ...
      {viewFor === e.id && (
        <div className="mt-4 pt-4" style={{ borderTop: `1px dashed ${C.line}` }}>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
            {[["Full name (entered by student)", e.requesterName || studentName(e.studentId)], ["SevisPass identity", `${studentName(e.studentId)} (${formatSevisId(e.studentId)})`], ["Student ID no.", formatSevisId(e.studentId)], ["Type of document requested", e.docType || "—"], ["Grade or course", e.program || "—"], ["Credential type", e.level || "—"], ["Starting year", e.startYear || (e.years || "—")], ["Ending year", e.endYear || "—"], ["Enrollment status", e.enrollment || "not yet verified"]].map(([label, value]) => (
            <div key={label}>
            ...
            <div className="mt-4 pt-4" style={{ borderTop: `1px dashed ${C.line}` }}>
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                ...
                <div>
                  ...
                  <div className="text-xs" style={{ fontFamily: MONO, color: C.gray }}>{formatSevisId(s.id)} · identity verified via SevisPass · {s.tier}</div>
                </div>
              </div>
