/* ============================================================
   PRESENTATION TIER — Institution dashboard.
   ============================================================ */
import React, { useState } from "react";
import { C, FONT, MONO } from "../theme.js";
import { Badge, Btn, Card, Field, Seal, SectionTitle } from "./ui.jsx";
import { computeRecordHash, readFileWithHash } from "../../application/crypto.js";
import { sampleTranscript } from "../../application/certificate.js";
import { now } from "../../application/textUtils.js";
import { CREDENTIAL_LEVELS, PERSONAS } from "../../data/seedData.js";
...
function InstitutionView({ user, institutions, entries, setEntries, records, setRecords, tokens, setTokens, log }) {
  const inst = institutions.find((i) => i.id === user.instId);
  if (!inst) {
  return (
    <Card>
      <p style={{ color: C.red }}>
        Institution profile not found. Please check user.instId and institution data.
      </p>
    </Card>
  );
}
  ...
  const [enroll, setEnroll] = useState({ program: "", years: "", level: "University Degree" });
  ...
  const runSearch = () => {
    ...
    setEnroll({ program: "", years: "", level: "University Degree" });
  };

  const createEnrollment = (student) => {
    const id = "e" + Math.random().toString(36).slice(2, 8);
    setEntries((es) => [...es, { id, studentId: student.id, institutionId: inst.id, institutionName: inst.name, type: "png", level: enroll.level, program: enroll.program, years: enroll.years, status: "png_verified", enrollment: "Active — enrolled" }]);
    ...
    return (
      <Card>
        ...
        <p className="text-sm mt-3" style={{ color: C.inkSoft }}>
          {inst.name} is awaiting platform approval by the system administrator. The dashboard unlocks once your registration (accreditation no. {inst.accreditationNo}) is approved.
        </p>
      </Card>
    );
      ...
      {certFor === e.id && (
        <div className="mt-4 pt-4" style={{ borderTop: `1px dashed ${C.line}` }}>
          <div className="grid grid-cols-2 gap-3">
            <Field label="GPA"><input className={inputCls} style={inputStyle} value={f.gpa} onChange={(ev) => setF({ ...f, gpa: ev.target.value })} placeholder="3.4" /></Field>
            <Field label="Class / award"><input className={inputCls} style={inputStyle} value={f.award} onChange={(ev) => setF({ ...f, award: ev.target.value })} placeholder="First Class Honours" /></Field>
            <Field label="Completion year"><input className={inputCls} style={inputStyle} value={f.year} onChange={(ev) => setF({ ...f, year: ev.target.value })} placeholder="2023" /></Field>
            <Field label="Courses completed (comma-separated)">
              <input className={inputCls} style={inputStyle} value={f.courses} onChange={(ev) => setF({ ...f, courses: ev.target.value })} placeholder="e.g. Programming I, Databases, Networks, Systems Analysis" />
            </Field>
            <Field label="Graduation status">
              <select className={inputCls} style={inputStyle} value={f.grad} onChange={(ev) => setF({ ...f, grad: ev.target.value })}>
          ...
          <Field label="Transcript / certificate text (the official document)">
            <textarea className={inputCls} style={{ ...inputStyle, minHeight: 110, fontFamily: MONO, fontSize: 12 }} value={f.doc} onChange={(ev) => setF({ ...f, doc: ev.target.value })} />
          </Field>
    ...
    <div>
      ...
      <Card>
        <div className="flex gap-2 flex-wrap">
          <input className={inputCls + " flex-1"} style={{ ...inputStyle, fontFamily: MONO, minWidth: 180 }} value={q} onChange={(ev) => setQ(ev.target.value)} placeholder="e.g. SP-1003" onKeyDown={(ev) => ev.key === "Enter" && runSearch()} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <Field label="Credential type">
                      <select className={inputCls} style={inputStyle} value={enroll.level} onChange={(ev) => setEnroll({ ...enroll, level: ev.target.value })}>
                        {CREDENTIAL_LEVELS.map((l) => <option key={l}>{l}</option>)}
                      </select>
                    </Field>
                    <Field label="Program"><input className={inputCls} style={inputStyle} value={enroll.program} onChange={(ev) => setEnroll({ ...enroll, program: ev.target.value })} placeholder="e.g. Grade 12 Certificate" /></Field>
                    <Field label="Years"><input className={inputCls} style={inputStyle} value={enroll.years} onChange={(ev) => setEnroll({ ...enroll, years: ev.target.value })} placeholder="e.g. 2022–2026" /></Field>
                  </div>
                  <Btn small kind="green" disabled={!enroll.program} onClick={() => createEnrollment(s)}>Register &amp; verify enrollment</Btn>
                </div>
              )}
            </div>
          );
      ...
      {queue.length === 0 && <Card><p className="text-sm" style={{ color: C.gray }}>Queue is empty. New student claims naming your institution appear here.</p></Card>}
      ...
      {history.map((r) => (
        <Card key={r.id}>
          ...
          {editFor === r.id && (
            <div className="mt-4 pt-4" style={{ borderTop: `1px dashed ${C.line}` }}>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Program"><input className={inputCls} style={inputStyle} value={ef.program} onChange={(ev) => setEf({ ...ef, program: ev.target.value })} /></Field>
                <Field label="GPA"><input className={inputCls} style={inputStyle} value={ef.gpa} onChange={(ev) => setEf({ ...ef, gpa: ev.target.value })} /></Field>
                <Field label="Class / award"><input className={inputCls} style={inputStyle} value={ef.award} onChange={(ev) => setEf({ ...ef, award: ev.target.value })} /></Field>
                <Field label="Completion year"><input className={inputCls} style={inputStyle} value={ef.year} onChange={(ev) => setEf({ ...ef, year: ev.target.value })} /></Field>
              </div>
              <Field label="Courses completed (comma-separated)">
                <input className={inputCls} style={inputStyle} value={ef.courses} onChange={(ev) => setEf({ ...ef, courses: ev.target.value })} />
              </Field>
              <Field label="Graduation status">
                <select className={inputCls} style={inputStyle} value={ef.grad} onChange={(ev) => setEf({ ...ef, grad: ev.target.value })}>
                  <option>Graduated</option>
                  <option>Enrolled — active</option>
                  <option>Completed — award pending</option>
                </select>
              ...
              <Field label="Transcript / certificate text">
                <textarea className={inputCls} style={{ ...inputStyle, minHeight: 110, fontFamily: MONO, fontSize: 12 }} value={ef.doc} onChange={(ev) => setEf({ ...ef, doc: ev.target.value })} />
              </Field>
