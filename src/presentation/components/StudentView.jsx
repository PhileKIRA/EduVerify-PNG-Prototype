...
function StudentView({ user, institutions, entries, setEntries, records, tokens, setTokens, checks, log, profiles, setProfiles }) {
  ...
  const shareBits = (q) => {
    ...
    const msg =
      `Verified academic credential — ${user.name} (${formatSevisId(user.id)})\n` +
    ...
    <>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Student ID no. (SevisPass — read-only)">
          <input className={inputCls} style={{ ...inputStyle(C), fontFamily: MONO, opacity: 0.75 }} value={formatSevisId(user.id)} readOnly />
        </Field>
      </div>
      ...
      <p className="text-xs mt-3" style={{ color: C.muted }}>
        Sent to your registrar's queue, matched to your SevisPass identity ({formatSevisId(user.id)}). Once the institution uploads and seals the requested record, it appears here automatically.
      </p>
    </>
  );
    ...
    <div>
      ...
      <Card>
        <div className="grid grid-cols-2 gap-3 text-sm mb-3">
          ...
          <div><div className="text-xs" style={{ color: C.gray }}>ID number (read-only)</div><div style={{ fontFamily: MONO, color: C.ink }}>{formatSevisId(user.id)}</div></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
