/* ============================================================
   PRESENTATION TIER — Student dashboard.
   ============================================================ */
import React, { useState } from "react";
import { C, FONT, MONO } from "../theme.js";
import { Badge, Btn, Card, Field, FingerprintStrip, Seal, SectionTitle, ShareLink } from "./ui.jsx";
import { buildCertificateHtml, sampleTranscript } from "../../application/certificate.js";
import { copyText, now, randToken, selectAllOnClick } from "../../application/textUtils.js";
import { CREDENTIAL_LEVELS } from "../../data/seedData.js";
...
function StudentView({ user, institutions, entries, setEntries, records, tokens, setTokens, checks, log, profiles, setProfiles }) {
  ...
  const html = await buildCertificateHtml(user.name, record);
  const blob = new Blob([html], { type: "text/html" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");

  a.href = url;a.download = `EduVerifyPNG_Record_${record.id}_${user.name.replace(/\s+/g, "_")}.html`;
  document.body.appendChild(a);
  a.click();

  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  log(`${user.name} downloaded official record ${record.id}.`);
...
<Field label="Your institution">
<select
className={inputCls}
style={inputStyle}
value={rf.institutionId}
onChange={(e)=>
setRf({...rf,institutionId:e.target.value})
}
>
...
value={rf.program}
onChange={(e)=>
setRf({...rf,program:e.target.value})
}
/>
</Field>
...
>
Send request to my institution
</Btn>
