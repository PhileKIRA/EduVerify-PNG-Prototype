...
import { formatSevisId } from "../../application/identity";
import { hashPassword, normalizeUsername, isPasswordAcceptable } from "../../application/credentials";
...
/* ============================================================ ADMIN */
function AdminView({ user, users = [], setUsers, institutions, setInstitutions, entries, setEntries, records, setRecords, events, log }) {
  ...
  const [regEditFor, setRegEditFor] = useState(null); // institution whose registrar is being reassigned
  const [regForm, setRegForm] = useState({ name: "", uid: "", username: "", password: "" });

  const registrarFor = (instId) => users.find((u) => u.role === "institution" && u.instId === instId) || null;

  const openReassign = (i) => {
    const current = registrarFor(i.id);
    setRegForm({ name: current?.name || i.registrarName || "", uid: current?.sub || i.registrarUid || "", username: current?.username || i.registrarUsername || "", password: "" });
    setRegEditFor(i.id);
  };

  const reassignUsernameTaken = (i) =>
    Boolean(regForm.username.trim()) &&
    users.some((u) => u.role === "institution" && u.instId !== i.id && normalizeUsername(u.username) === normalizeUsername(regForm.username));

  const canSaveReassign = (i) =>
    Boolean(regForm.username.trim()) &&
    !reassignUsernameTaken(i) &&
    isPasswordAcceptable(regForm.password);

  /* Reassign the person responsible for an institution. The new registrar signs
     in with the username/password set here; the previous registrar loses access.
     A SevisPass UID may still be recorded for SevisPass-linked deployments. */
  const saveReassign = async (i) => {
    const uid = regForm.uid.trim();
    const name = regForm.name.trim() || `${i.name} Registrar`;
    const username = normalizeUsername(regForm.username);
    if (!canSaveReassign(i)) return;
    const passwordHash = await hashPassword(regForm.password);
    setInstitutions((xs) => xs.map((x) => (x.id === i.id ? { ...x, registrarUid: uid, registrarName: name, registrarUsername: username } : x)));
    setUsers((us) => {
      const withoutOld = us.filter((u) => !(u.role === "institution" && u.instId === i.id));
      return [...withoutOld, { id: uid ? `sevis:${uid}` : `reg:${i.id}`, role: "institution", name, instId: i.id, sub: uid || `reg:${i.id}`, tier: "Registrar — username sign-in", username, passwordHash }];
    });
    log(`Admin reassigned the registrar for ${i.name} to ${name} (username ${username}).`);
    setRegEditFor(null);
  };
    ...
    <div>
      ...
      {tab === "institutions" && (
        <>
          ...
          {institutions.map((i) => {
            const registrar = registrarFor(i.id);
            const editing = regEditFor === i.id;
            return (
              <Card key={i.id}>
                ...
                <div className="mt-3 pt-3 flex items-center justify-between gap-2 flex-wrap" style={{ borderTop: `1px dashed ${C.line}` }}>
                  <div className="text-xs" style={{ color: C.gray }}>
                    Registrar responsible:{" "}
                    <span style={{ color: C.ink, fontWeight: 600 }}>{registrar ? registrar.name : i.registrarName || "—"}</span>
                    {(registrar?.username || i.registrarUsername) ? (
                      <span style={{ fontFamily: MONO, color: C.muted }}> · username {registrar?.username || i.registrarUsername}</span>
                    ) : (registrar?.sub || i.registrarUid) ? (
                      <span style={{ fontFamily: MONO, color: C.muted }}> · UID {registrar?.sub || i.registrarUid}</span>
                    ) : null}
                  </div>
                  {i.status === "approved" && !editing && (
                    <Btn small kind="ghost" onClick={() => openReassign(i)}>Change registrar</Btn>
                  )}
                </div>

                {i.status === "approved" && editing && (
                  <div className="mt-3 pt-3" style={{ borderTop: `1px dashed ${C.line}` }}>
                    <div className="text-xs font-semibold mb-3" style={{ color: C.goldDeep, letterSpacing: "0.06em" }}>REASSIGN REGISTRAR</div>
                    <div className="grid gap-2">
                      <div>
                        <div className="text-xs mb-1" style={{ color: C.gray }}>New registrar name (person or office)</div>
                        <input className={inputCls} style={inputStyle(C)} value={regForm.name} onChange={(e) => setRegForm({ ...regForm, name: e.target.value })} placeholder="e.g. Deputy Registry Office" />
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <div className="text-xs mb-1" style={{ color: C.gray }}>New sign-in username</div>
                          <input className={inputCls} style={inputStyle(C)} value={regForm.username} autoComplete="off" onChange={(e) => setRegForm({ ...regForm, username: e.target.value })} placeholder="e.g. upng-deputy" />
                        </div>
                        <div>
                          <div className="text-xs mb-1" style={{ color: C.gray }}>New password (min 6 chars)</div>
                          <input className={inputCls} style={inputStyle(C)} type="password" value={regForm.password} autoComplete="new-password" onChange={(e) => setRegForm({ ...regForm, password: e.target.value })} placeholder="set a new password" />
                        </div>
                      </div>
                      {reassignUsernameTaken(i) && <p className="text-xs" style={{ color: C.red }}>That username is already used by another institution.</p>}
                      {regForm.password && !isPasswordAcceptable(regForm.password) && <p className="text-xs" style={{ color: C.amber }}>Password must be at least 6 characters.</p>}
                      <div>
                        <div className="text-xs mb-1" style={{ color: C.gray }}>SevisPass UID (optional)</div>
                        <input className={inputCls} style={inputStyle(C)} value={regForm.uid} onChange={(e) => setRegForm({ ...regForm, uid: e.target.value })} placeholder="only for SevisPass-linked deployments" />
                      </div>
                    </div>
                    <p className="text-xs mt-2" style={{ color: C.muted }}>
                      {i.name}'s new registrar signs in with this username and password. The previous registrar loses access to this institution's portal.
                    </p>
                    <div className="flex gap-2 mt-3">
                      <Btn small kind="gold" disabled={!canSaveReassign(i)} onClick={() => saveReassign(i)}>Save registrar</Btn>
                      <Btn small kind="ghost" onClick={() => setRegEditFor(null)}>Cancel</Btn>
                    </div>
                  </div>
                )}
              </Card>
              ...
              <Card key={e.id}>
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="grid gap-0.5">
                    <div className="font-bold text-sm" style={{ color: C.ink }}>{studentName(e.studentId)} <span style={{ fontFamily: MONO, fontWeight: 400, color: C.gray }}>({formatSevisId(e.studentId)})</span></div>
                    ...
                    <Modal onClose={() => setViewReview(null)} maxWidth={720}>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                        {[
                          ...
                          ["Student ID (SevisPass)", formatSevisId(e.studentId)],
