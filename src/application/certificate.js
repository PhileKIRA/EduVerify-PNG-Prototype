...
function buildCertificateHtml(studentName, r) {
  ...
  const qr = qrToSvgString("EDUVERIFY-PNG|SHA256:" + r.hash); // real, scannable QR encoding the record hash
  return `<!DOCTYPE html>
...
${s.documentType ? `<tr><td><b>Document type</b></td><td>${esc(s.documentType)}</td></tr>` : ""}
${s.startYear && s.endYear ? `<tr><td><b>Period of study</b></td><td>${esc(s.startYear === s.endYear ? s.startYear : s.startYear + "–" + s.endYear)}</td></tr>` : ""}
...
export { buildCertificateHtml, sampleTranscript };
